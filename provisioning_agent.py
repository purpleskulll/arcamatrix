...
def update_middleware_mapping(username, sprite_url):
    """Update middleware.ts with new customer mapping, commit and push to trigger Vercel deploy."""
    try:
        # Pull latest
        ...
        if f"'{username}'" in content:
            log(f"Middleware mapping for {username} already exists, skipping")
            return True

        # Find the customerMappings object and add new entry
        # Pattern: after the last existing entry, before the closing };
        ...
        # Add new mapping before the closing brace
        new_entry = f"  '{username}': '{sprite_url}',"
        # Insert after the last line that has a mapping entry
        ...
        log(f"Middleware mapping added for {username}, pushed to trigger Vercel deploy")
        return True

    except Exception as e:
        log(f"Failed to update middleware mapping: {e}")
        # Reset any local changes on failure
        subprocess.run(["git", "checkout", "--", "src/middleware.ts"], cwd=REPO_DIR, capture_output=True)
        return False

def remove_middleware_mapping(username):
    """Remove a customer mapping from middleware.ts, commit and push."""
    try:
        subprocess.run(["git", "pull", "--rebase"], cwd=REPO_DIR, capture_output=True, timeout=30)
        ...
        if f"'{username}'" not in content:
            log(f"Middleware mapping for {username} not found, skipping removal")
            return True

        # Remove the line containing this username's mapping
        lines = content.split('\n')
        new_lines = [l for l in lines if f"'{username}'" not in l]
        content = '\n'.join(new_lines)
        MIDDLEWARE_PATH.write_text(content)

        subprocess.run(["git", "add", "src/middleware.ts"], cwd=REPO_DIR, capture_output=True, timeout=10)
        result = subprocess.run(
            ["git", "commit", "-m", f"Remove customer mapping: {username}"],
            cwd=REPO_DIR, capture_output=True, text=True, timeout=10
        )
        if result.returncode != 0:
            return False

        subprocess.run(["git", "push"], cwd=REPO_DIR, capture_output=True, text=True, timeout=30)
        log(f"Middleware mapping removed for {username}")
        return True

    except Exception as e:
        log(f"Failed to remove middleware mapping: {e}")
        subprocess.run(["git", "checkout", "--", "src/middleware.ts"], cwd=REPO_DIR, capture_output=True)
        return False
...
def provision_sprite(task_id, task_data):
    ...
    try:
        ...
        # Register customer mapping via API (backup, /tmp is ephemeral)
        log("Registering customer-sprite mapping via API...")
        mapping_result = register_customer_mapping(username, sprite_internal_url, sprite_name)
        if mapping_result:
            log(f"Customer mapping registered via API for {username}")

        # Update middleware.ts in git and push (triggers Vercel deploy - permanent mapping)
        log("Updating middleware mapping in git...")
        middleware_ok = update_middleware_mapping(username, sprite_internal_url)
        if middleware_ok:
            log(f"Middleware mapping updated and deployed for {username}")
        else:
            log(f"WARNING: Failed to update middleware mapping - customer URL may not work until manually added")
...
def handle_recycle(task_id, task_data):
    ...
    try:
        ...
        # Remove customer mapping via API
        try:
        ...
        # Remove middleware mapping from git
